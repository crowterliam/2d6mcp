...
| `parse_character` | Parse a character sheet (`file_path` or `sheet_text`). |
...
4. Parse existing sheets: `parse_character(file_path)` or `parse_character(sheet_text)`

### BYOD Management
1. Search personal content: `query_local_byod("search term")` — names the collection when possible (for example include "collection-a") so matching folders are indexed, then searched. Pass `root`/`relative_path` (for example `parent/line`) so sibling editions are not walked.
...
4. Live companion: `ingest_live_transcript(action: poll, session_id)` — companion SQLite or NDJSON; loopback/mic capture is out of scope
