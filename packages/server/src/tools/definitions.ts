...
export function getToolDefinitions(options: ToolDefinitionOptions = {}): Tool[] {
  const tools: Tool[] = [
      ...
      inputSchema: {
        ...
        properties: {
          ...
          intent: {
            type: "string",
            description:
              "Classified intent: question, ruling, action, narration, discussion, chronicle, or beat. chronicle/beat writes a provisional chronicle beat when the session has table_label.",
          },
        },
    ...
    },
    {
      ...
      description:
        "Ingest a live companion transcript into a session. Reads new segments since a per-session cursor and logs them via log_transcript (source=voice, intent=narration). Sources: companion_sqlite (external SQLite with meetings + segments tables; not bundled), ndjson_file, or watch_dir. action: poll (alias ingest), status, or reset_cursor. Paths must be under the project root, BYOD_PATH, LIVE_TRANSCRIPT_ALLOW_PATHS, or LIVE_TRANSCRIPT_DB.",
      inputSchema: {
        type: "object",
        properties: {
          action: {
          ...
          source: {
            type: "string",
            enum: ["companion_sqlite", "ndjson_file", "watch_dir"],
            description:
          ...
          path: {
            type: "string",
            description:
              "SQLite DB, NDJSON/JSONL file, or watch directory. Optional for sqlite when LIVE_TRANSCRIPT_DB is set or a companion DB is auto-discovered.",
          },
          meeting_id: {
            type: "string",
            description:
          ...
          chronicle_hints: {
            type: "boolean",
            description:
              "If true, write provisional chronicle note beats for ingested segments when the session has table_label. Default false. Never auto-confirms.",
            default: false,
          },
        },
        required: ["session_id"],
    ...
    },
    {
      name: "chronicle",
      description:
        "Table-scoped chronicle: threads, beats, entities, links, hooks, brief (no LLM), search, promote, extract_candidates, and optional markdown export. Extracts are always provisional; only promote confirms. Scoped by table_label.",
      inputSchema: {
        type: "object",
        properties: {
          action: {
            ...
            description: "Chronicle action. brief needs no LLM. extract_candidates is last-resort LLM only when use_llm is true.",
          },
          table_label: {
            type: "string",
            description: "Durable table tag (e.g. table-a, campaign-label). Required for most actions.",
          },
          id: { type: "string", description: "Existing row id (upsert/get/promote/unlink)" },
          ids: {
            type: "array",
            items: { type: "string" },
            description: "Row ids to promote (beats or entities)",
          },
          title: { type: "string", description: "Thread title" },
          status: { type: "string", description: "Thread status (open|dormant|resolved|abandoned) or hook status (ready|used|burned)" },
          priority: { type: "integer", description: "Thread priority (higher first)" },
          summary: { type: "string", description: "Thread summary or extract source text" },
          tags: { type: "array", items: { type: "string" }, description: "Thread tags" },
          opened_session_id: { type: "string" },
          resolved_session_id: { type: "string" },
          thread_id: { type: "string" },
          session_id: { type: "string" },
          kind: {
            type: "string",
            description: "Beat kind: reveal, decision, combat, travel, rumour, loot, consequence, note",
          },
          text: { type: "string", description: "Beat, hook, or extract text" },
          source: {
            type: "string",
            description: "Beat source: manual, from_transcript, from_summary, agent",
          },
          confidence: { type: "string", description: "confirmed or provisional. Extracts are always stored as provisional." },
          since: { type: "integer", description: "list_beats: unix ms lower bound" },
          type: { type: "string", description: "Entity type: person, place, faction, item, ship, other" },
          name: { type: "string", description: "Entity name" },
          aliases: { type: "array", items: { type: "string" } },
          notes: { type: "string" },
          last_seen_session_id: { type: "string" },
          from_id: { type: "string" },
          to_id: { type: "string" },
          rel: { type: "string", description: "Link relation" },
          note: { type: "string", description: "Link note" },
          query: { type: "string", description: "search query (LIKE/AND, same as search_transcript)" },
          limit: { type: "integer" },
          write: {
            type: "boolean",
            description: "extract_candidates: store provisional rows (default false)",
            default: false,
          },
          use_llm: {
            type: "boolean",
            description: "extract_candidates: use local LLM as last resort. Default false (heuristic).",
            default: false,
          },
          path: {
            type: "string",
            description: "export: allowlisted local markdown path (project, BYOD, live-transcript, or CHRONICLE_EXPORT_ALLOW_PATHS)",
          },
        },
        required: ["action"],
      },
