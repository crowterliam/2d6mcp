...
import { getByodDatabase, indexChunks, rebuildByodFts, isByodFtsStale, getStoredFileHash, markFileFailed, FAILED_HASH } from "../byod/search.js";
...
export async function syncByodIndex(config: Config): Promise<SyncResult> {
  ...
  while (i < files.length) {
    ...
    if (elapsed >= config.byodSyncTimeoutMs) {
      if (indexedFiles > 0 || isByodFtsStale(db)) {
        rebuildByodFts(db);
      }
